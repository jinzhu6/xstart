...
// Classes GregorianDate, JulianDate, IsoDate, IslamicDate and HebrewDate
...
void GetGregorianFromHijri(int& day, int& month, int& year, int offset) {
	IslamicDate hijri(month, day, year);
	GregorianDate gregorian(hijri+offset);
	day = gregorian.day;
...
void GetHijriFromGregorian(int& day, int& month, int& year, int offset) {
	...
	IslamicDate hijri(gregorian + offset);
	day = hijri.day;
	month = hijri.month;
	year = hijri.year;
};
