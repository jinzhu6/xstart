...
coDword ConvertToArabic(coDword* normal, int length, coDword* result, int maxResult) {
	CharRep crep;
	CombCharRep combcrep;
	int writeCount = 0;

	for (int i = 0; i < length; i++) {
		coDword current = normal[i];
		...
		if (CharacterMapContains(current)) {
			...
			if ((nextID >= length) || !CharacterMapContains(next = normal[nextID])
			...
			if (current == 0x0644 && next != NIL && (next == 0x0622 || next == 0x0623
				...
				if (prev != NIL) {
					result[writeCount++] = combcrep.mFinal;
				} else {
					result[writeCount++] = combcrep.mIsolated;
				}
				i++;
				continue;
			}
			...
			if (prev != NIL && next != NIL && crep.mMedial != NIL) {
				result[writeCount++] = crep.mMedial;
				continue;
				/* Final */
			} else if (prev != NIL && crep.mFinal != NIL) {
				result[writeCount++] = crep.mFinal;
				continue;
				/* Initial */
			} else if (next != NIL && crep.mInitial != NIL) {
				result[writeCount++] = crep.mInitial;
				continue;
			}
			/* Isolated */
			result[writeCount++] = crep.mIsolated;
		} else {
			result[writeCount++] = current;
		}
	}

	result[writeCount] = NIL;
