...
public:
	...
	inline void Sys_SetState(State a_state) {
		m_state = a_state;
	}
	...
	inline const gmStackFrame* GetFrame() const {
		return m_frame;
	}
	...
	inline gmVariable &Pop() {
		return m_stack[--m_top];
	}
	inline void Push(const gmVariable &a_variable) {
		m_stack[m_top++] = a_variable;
	}
	...
	inline const gmuint8* GetInstruction() const {
		return m_instruction;
	}
	inline State GetState() const {
		return m_state;
	}
	inline gmuint32 GetTimeStamp() const {
		return m_timeStamp;
	}
	inline gmuint32 GetThreadTime() const {
		return m_machine->GetTime() - m_startTime;
	}
	inline void Sys_SetTimeStamp(gmuint32 a_timeStamp) {
		m_timeStamp = a_timeStamp;
	}
	inline void Sys_SetStartTime(gmuint32 a_startTime) {
		m_startTime = a_startTime;
	}

	/// \brief GetSystemMemUsed will return the number of bytes allocated by the system.
	inline unsigned int GetSystemMemUsed() const {
		return (m_size * sizeof(gmVariable)) + sizeof(this);
	}
